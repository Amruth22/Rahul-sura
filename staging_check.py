...
def get_credentials_from_edrc():
    edrc_path = os.path.expanduser('~/.ederc')  # Using ~/.ederc instead of ~/.edrc
    creds = {}
...
def create_auth_header(method, url, creds):
    ...
    # Parse just the path and query from the full URL for signing
    parsed_url = url.split('://')[-1]
    if '/' in parsed_url:
        domain, path_and_query = parsed_url.split('/', 1)
        path_and_query = '/' + path_and_query
    else:
        path_and_query = '/'
    
    data_to_sign = f"{method}\thttps://{domain}\t{path_and_query}\t"
